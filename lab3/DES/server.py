...
def des_encrypt_block(block: int, subkeys: list, debug: list) -> int:
...
def text_to_block(text: str, pos: int) -> int:
    """Convert 8 characters (or less) to 64-bit block, pad with spaces"""
    chunk = text[pos:pos+8].ljust(8)
    block = 0
...
def main():
    ...
    print("\n=== Encryption side (Server) ===")
    message = input("Message/Plaintext: ").strip()
    key = input("Key (exactly 8 chars): ").strip()
    ...
    i = 0
    while i < len(message):
        block = text_to_block(message, i)
        debug = []
        encrypted = des_encrypt_block(block, subkeys, debug)
        debug_all.append(f"BLOCK {len(debug_all)+1}:\n" + "\n".join(debug))
        ciphertext_hex += f"{encrypted:016X}"
        i += 8

    print("\n".join(debug_all))
    print(f"\nCipher Text (sent to client): {ciphertext_hex}")

    # Send ONLY ciphertext to client
    conn.send((ciphertext_hex + "\n").encode())

    print("\nCiphertext sent to client.")
    print("Waiting for client to decrypt...\n")
